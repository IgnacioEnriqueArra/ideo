...
import { ArrowLeft, GitFork, Heart, Bookmark, Share, MoreHorizontal, BadgeCheck, MessageSquare } from 'lucide-react';
...
const RecursiveForkThread = ({ fork, onUserClick, onReplyClick, depth = 0 }: any) => {
   ...
   return (
       ...
       >
         {/* Subtle connection line for Web3 vibe */}
         {depth > 0 && (
           <div className="absolute left-6 top-10 bottom-0 w-px bg-gradient-to-b from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
         )}
         
         <BranchCard branch={fork} onUserClick={onUserClick} onReply={() => onReplyClick(fork)} />
         
         {fork.forks && fork.forks.length > 0 && (
             <div className={`${shouldIndent ? 'border-l-2 border-primary/10 ml-6 pl-2' : 'ml-0 pl-0'} mt-1 transition-colors duration-300`}>
...
export const IdeaDetail: React.FC<IdeaDetailProps> = ({ ideaId, onBack, onUserClick }) => {
  ...
  if (!idea) return <div className="p-8 text-center font-mono text-primary bg-primary/5 m-4 rounded-xl">Idea not found</div>;
    ...
    <>
      ...
      >
        ...
        <div className="p-5 border-b border-gray-100 relative overflow-hidden">
          {/* Subtle decoration background blob */}
        ...
        <div className="px-5 py-4 border-b border-gray-100 bg-white/70 sticky top-[68px] z-10 backdrop-blur-xl shadow-sm">
          <div className="flex items-center gap-3">
             ...
             <button 
               onClick={openRootFork}
               className="flex-1 bg-white border border-gray-200 shadow-sm rounded-2xl px-4 py-3 text-sm text-gray-400 text-left hover:border-primary/40 focus:outline-none transition-all font-mono"
             >
               Attach a fork to this...
             </button>
             <button 
               onClick={openRootFork}
               className="bg-primary text-white font-black px-6 py-3 rounded-2xl shadow-lg shadow-primary/20 hover:shadow-primary/40 active:scale-95 transition-all text-sm"
             >
               Fork
             </button>
          </div>
        ...
        {/* Forks List */}
        <div className="divide-y divide-gray-100/50 pb-32">
          <AnimatePresence>
